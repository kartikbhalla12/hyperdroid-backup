...
/*function bigpix_custom_excerpt($text) {  // custom 'read more' link
  global $post;
  $ismore = @strpos( $post->post_content, '<!–more–>');
  if ($ismore) {
  $excerpt = '<p>' . strip_tags($text) . '… <a class="excerpt-read-more" href="'. get_permalink($post->ID) . '" title="'. __( 'Read ', 'BIGPIX' ) . get_the_title($post->ID).'">'. __( 'Read more &raquo;', 'BIGPIX' ) .'</a></p>';
  return $excerpt;
  }
  else{
...
add_filter('the_excerpt', 'bigpix_custom_excerpt');*/
...
if ( ! function_exists( 'BIGPIX_apply_color' ) ) :
  function BIGPIX_apply_color() {
   ...
   ?>
  <style>
      .blog-list .item .hover-ovelay:hover{background: <?php echo esc_attr( get_theme_mod('BIGPIX_color_settings') ); ?>!important;}
      body .pagination li a{color: <?php echo esc_attr( get_theme_mod('BIGPIX_color_settings') ); ?>;}
      #main-navigation,.full-top-area,footer.footer[role="contentinfo"]{background: <?php echo esc_attr( get_theme_mod('BIGPIX_color_settings') ); ?>; }
    </style>
   <?php
    }
    ?>
  <?php  if ( get_theme_mod('BIGPIX_color_settings_2') ) {
   ?>
    <style>
      ...
      .entry-content p,body .byline.blog a,body .info p.author-desc, body .related ul li .related-info p,body .comment time a,body .comment_content p,footer.footer[role="contentinfo"] .wp-caption p.wp-caption-text{color:<?php echo esc_attr( get_theme_mod('BIGPIX_color_settings_2') ); ?>;}
      body .pagination li:first-child{border-left: 2px solid <?php echo esc_attr( get_theme_mod('BIGPIX_color_settings_2') ); ?>; }
      body .pagination li{border: 2px solid <?php echo esc_attr( get_theme_mod('BIGPIX_color_settings_2') ); ?>;}
    </style>
   ...
   ?>
    <style>
      ...
      ,.error404 .searchform input[type="submit"],.footer-widgets .widget #wp-calendar caption,.wp-caption,#main-navigation div#close{background:<?php echo esc_attr( get_theme_mod('BIGPIX_color_settings_3') ); ?>; }
      .blog-list li .excerpt p a,.blog-list .item a[rel="author"],body .info p.author-desc a,body .related ul li a
      ,body .comment-reply-link,.blog-list .item h2:hover,#main-navigation .widgettitle,.social-icons a .fa:hover,footer.footer[role="contentinfo"] a:hover,.entry-content p a,#main-navigation a:hover,.no-menu-widgets p{color:<?php echo esc_attr( get_theme_mod('BIGPIX_color_settings_3') ); ?>;}
      .entry-content blockquote{border-left: 3px solid <?php echo esc_attr( get_theme_mod('BIGPIX_color_settings_3') ); ?>;}
    </style>
...
add_action( 'tgmpa_register', 'BIGPIX_register_required_plugins' );
/**
...
function BIGPIX_register_required_plugins() {
    ...
    $config = array(
        ...
        'strings'      => array(
            ...
            'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.','BIGPIX' ), // %1$s = plugin name(s).
            'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.','BIGPIX' ), // %1$s = plugin name(s).
            'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.' ,'BIGPIX'), // %1$s = plugin name(s).
            'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.' ,'BIGPIX'), // %1$s = plugin name(s).
            'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.' ,'BIGPIX'), // %1$s = plugin name(s).
            'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.' ,'BIGPIX'), // %1$s = plugin name(s).
            'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.' ,'BIGPIX'), // %1$s = plugin name(s).
            'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.' ,'BIGPIX'), // %1$s = plugin name(s).
            'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins' ,'BIGPIX'),
            'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins' ,'BIGPIX'),
